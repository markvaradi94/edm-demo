...
public class TwitterElasticQueryClient implements ElasticQueryClient<TwitterIndexModel> {
    ...
    public TwitterIndexModel getIndexModelById(String id) {
        ...
        SearchHit<TwitterIndexModel> searchResult =
                ofNullable(elasticsearchOperations.searchOne(query, TwitterIndexModel.class, IndexCoordinates.of(elasticConfigData.getIndexName())))
    ...
    public List<TwitterIndexModel> getIndexModelByText(String text) {
        ...
        return search(query, "{} document(s) with text {} retrieved successfully", text);
    }

    @Override
    public List<TwitterIndexModel> getAllIndexModels() {
        Query query = elasticQueryUtil.getSearchQueryForAll();
        return search(query, "{} document(s) with text {} retrieved successfully");
    }
    ...
    private List<TwitterIndexModel> search(Query query, String logMessage, Object... logParams) {
        SearchHits<TwitterIndexModel> searchResult =
                elasticsearchOperations.search(query, TwitterIndexModel.class, IndexCoordinates.of(elasticConfigData.getIndexName()));
